\subsection{A solução}
  ...
  Sem muito esforço cognitivo chegamos a solução apresentada na \figref{primeira-solucao}.
  ...
  Essas reflexões levam aos seguintes artefatos derivados da refatoração:
	...
	% \begin{figure}[h]
    % \centering
    % \caption{Implementação do código do backend}%
    % \efbox{
      % \makebox[\textwidth]{
        % \subfloat[\centering Main.java]{{
          % \includegraphics
            % [width=6cm]
            % {main.java.png}
        % }}
        % \qquad
        % \subfloat[\centering Usuario.java]{{
          % \includegraphics
            % [width=4cm]
            % {usuario.java.png}
        % }}
    ...
    % }
    % Fonte: Próprio autor
    % \label{fig:implementacao-backend}
	% \end{figure}
