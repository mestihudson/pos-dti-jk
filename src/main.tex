...
\begin{document}
  ...
  \section{Arquitetura de Software}
    ...
    propriedades que lhe são comuns entre ela e sua "aparentada" e seu
    ...
    denominação da chamada Arquitetura de Software que, na ausência de
    uma expressão mais apropriada, toma emprestado a alcunha para
    conceituar uma abordagem com um alto nível de abstração, que "a
    matéria dos dígitos binários" exige.
    ...
    Etimologicamente, a palavra remete à composição grega de \emph{ARKHEIN} -
    que significa \emph{comandar} em português - com \emph{TEKTON} - que por
    sua vez quer dizer \emph{artesão ou construtor}\footnote{\cite{
    Etimologia2019}}.

    Daí, por aproximação, diz tratar-se dos dítames direcionadores de uma
    determinada construção. Ora, não resta dúvida, assim, que Arquitetura de
    Software são os preceitos orientadores de um sistema computacional.

    \subsection{Arquitetura Limpa}

      Vejamos, se ao vislumbrar determinada edificação é possível inferir algumas
      ...
      Tal qual uma Arquitetura Limpa "grita"\cite[p. 196]{Martin2018} qual a sua
      motivação - uma casa, uma biblioteca, uma fábrica, etc. - uma Arquitetura
      de Software Limpa diz \emph{o quê é} e emph{o que pode ser feito em seu
      contexto}.
  ...
  \section{Como TDD auxilia na processo de chegada a uma Arquitetura Limpa?}
    ...
    soft - em português, flexível, suave - é um bem intangível, cuja
    construção, guardadas os devidos contextos e proporções, não oferece os
    mesmos riscos, talvez, irreparáveis que a primeira.

    Desenvolvimento guiado por testes usa uma abordagem que permite definir as
    funções essenciais do sistema, implementá-las da maneira mais simplória
    conhecida e através de sucessivas modificações, amparadas por uma rede de
    proteção que previne a regressão, alcançar um modelo que diz, claramente,
    quais seus objetivos e permite a sustentável evolução do sistema.
